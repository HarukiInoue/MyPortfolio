...
description: End-to-end workflow for producing a playful claymation-style explainer/ad video: storyboard → character reference images → scene stills → Veo image-to-video clips → single TTS voiceover → FFmpeg fast-cut assembly → webpage artifact. Captures the proven pipeline (and its gotchas) for character/scene-consistent multi-shot animated shorts.
source: https://github.com/alexmcdonnell-airtable/hyperagent-public-skills/blob/main/skill-claymation-explainer.json
---
